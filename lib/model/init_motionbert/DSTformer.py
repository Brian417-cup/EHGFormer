...
# 这个块在Block中只用到了spatial和temporal两个模式
class Attention(nn.Module):
    ...
    def reshape_T(self, x, seqlen=1, inverse=False):
        if not inverse:
            ...
            # 最后的forward_coupling时的attn@v后进入这块
            TN, C = x.shape[-2:]
...
class DSTformer(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=256, dim_rep=512,
        ...
        self.att_fuse = att_fuse
        # 如果对st和ts两条支路做混合的话，这里选择对每层的st和tsconcat后的 [batchsize*T,V,2C] -> [batchsize*T,V,C]
        if self.att_fuse:
...
if __name__ == '__main__':
    ...
    # 这里在pytorch中1*1的nn.Conv2d 和 nn.Linear作用的维度是不一样的
    # 在使用nn.Linear时，pytorch会将除了最后一个维度，其他前面维度自动作为.view(-1,in_c),经过计算得到.view(-1,out_c)后再重新拉回原来的尺寸
    # 而nn.Conv2d将自动找到 (N,C,H,W) 中的C 做 c_in到 c_out的映射
    layer = nn.Linear(8, 5)
    # layer = nn.Conv2d(4, 8, kernel_size=1, stride=1, padding=0)
    y = layer(x)
