...
class DataReaderH36M(object):
    ...
    def read_2d(self):
        ...
        # map to [-1, 1]
        for idx, camera_name in enumerate(self.dt_dataset['train']['camera_name'][::self.sample_stride]):
        ...
        for idx, camera_name in enumerate(self.dt_dataset['test']['camera_name'][::self.sample_stride]):
            if camera_name == '54138969' or camera_name == '60457274':
                res_w, res_h = 1000, 1002
            elif camera_name == '55011271' or camera_name == '58860488':
                res_w, res_h = 1000, 1000
    ...
    def read_hw(self):
        if self.test_hw is not None:
            return self.test_hw
        test_hw = np.zeros((len(self.dt_dataset['test']['camera_name'][::self.sample_stride]), 2))
        for idx, camera_name in enumerate(self.dt_dataset['test']['camera_name'][::self.sample_stride]):
            if camera_name == '54138969' or camera_name == '60457274':
                res_w, res_h = 1000, 1002
            elif camera_name == '55011271' or camera_name == '58860488':
    ...
    def get_split_id(self):
        # If exists split_id,it directly returns
    ...
    def get_sliced_data(self):
        ...
        split_id_train, split_id_test = self.get_split_id()  # range for [start,end) in temporal index
    ...
    def denormalize(self, test_data):
        ...
        n_frames = test_data.shape[1]
        test_hw = self.get_hw()
        data = test_data.reshape([n_clips, n_frames, 17, -1])

        assert len(data) == len(test_hw)  # test data shape is (2228,243,17,3), test hw shape is (2228,2)
