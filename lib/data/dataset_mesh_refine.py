...
class SMPLDataset(Dataset):
    def __init__(self, args, data_split, dataset):  # data_split: train/test; dataset: h36m, coco, pw3d
        ...
        if dataset == "h36m":
            datareader = DataReaderH36M(n_frames=self.clip_len, sample_stride=args.sample_stride,
                                        data_stride_train=args.data_stride, data_stride_test=self.clip_len,
                                        dt_root=args.data_root, dt_file=args.dt_file_h36m)
        elif dataset == "coco":
            datareader = DataReaderMesh(n_frames=1, sample_stride=args.sample_stride, data_stride_train=1,
                                        data_stride_test=1, dt_root=args.data_root, dt_file=args.dt_file_coco,
                                        res=[640, 640])
        elif dataset == "pw3d":
            datareader = DataReaderMesh(n_frames=self.clip_len, sample_stride=args.sample_stride,
                                        data_stride_train=args.data_stride, data_stride_test=self.clip_len,
                                        dt_root=args.data_root, dt_file=args.dt_file_pw3d, res=[1920, 1920])
        else:
        ...
        pred_smpl_pose_train = dt['train']['pred_smpl_pose'][split_id_train]  # (N, T, 72)
        pred_smpl_shape_train = dt['train']['pred_smpl_shape'][split_id_train]  # (N, T, 10)

        smpl_pose_test = dt['test']['smpl_pose'][split_id_test]  # (N, T, 72)
        smpl_shape_test = dt['test']['smpl_shape'][split_id_test]  # (N, T, 10)

        pred_smpl_pose_test = dt['test']['pred_smpl_pose'][split_id_test]  # (N, T, 72)
        pred_smpl_shape_test = dt['test']['pred_smpl_shape'][split_id_test]  # (N, T, 10)

        self.motion_smpl_3d = {'train': {'pose': smpl_pose_train, 'shape': smpl_shape_train},
                               'test': {'pose': smpl_pose_test, 'shape': smpl_shape_test}}[data_split]
        self.pred_motion_smpl_3d = {'train': {'pose': pred_smpl_pose_train, 'shape': pred_smpl_shape_train},
                                    'test': {'pose': pred_smpl_pose_test, 'shape': pred_smpl_shape_test}}[data_split]
        self.smpl = SMPL(
            args.data_root,
            batch_size=1,
        )
...
class MotionSMPL(SMPLDataset):
    ...
    def __getitem__(self, index):
        ...
        pred_motion_smpl_pose = self.pred_motion_smpl_3d['pose'][index].reshape(-1, 24, 3)  # (T,24,3)
        pred_motion_smpl_shape = self.pred_motion_smpl_3d['shape'][index]  # (T,10)
        ...
        if self.data_split == "train":
            if self.flip and random.random() > 0.5:  # Training augmentation - random flipping
                ...
                pred_motion_smpl_pose = flip_thetas(pred_motion_smpl_pose)
        ...
        pred_motion_smpl_pose = torch.from_numpy(pred_motion_smpl_pose).reshape(-1, 72).float()
        pred_motion_smpl_shape = torch.from_numpy(pred_motion_smpl_shape).reshape(-1, 10).float()
        motion_smpl_3d = {
            'theta': motion_theta,  # smpl pose and shape
            'kp_3d': motion_3d_reg,  # 3D keypoints
            'verts': motion_verts,  # 3D mesh vertices
            'pred_smpl_pose': pred_motion_smpl_pose,
            'pred_smpl_shape': pred_motion_smpl_shape
        }
