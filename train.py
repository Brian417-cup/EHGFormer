...
def train_with_config(args, opts):
    ...
    datareader = DataReaderH36M(n_frames=args.clip_len, sample_stride=args.sample_stride,
                                ...
                                dt_root=osp.join(args.data_root, args.subset_list[0]), dt_file=args.dt_file)
    min_loss = 100000
        ...
        for epoch in range(st, args.epochs):
            ...
            if args.no_eval:
                ...
                print('[%d] time %.2f lr %f 3d_train %f e1 %f e2 %f' % (
                    ...
                    losses['3d_pos'].avg,
                    e1, e2))
                logging.info('[%d] time %.2f lr %f 3d_train %f e1 %f e2 %f' % (
                    epoch + 1,
                    elapsed,
                    lr,
                    losses['3d_pos'].avg,
            ...
            save_checkpoint(chk_path_latest, epoch + 1, lr, optimizer, model_pos, min_loss)

            # save frequency checkpoint
            if (epoch + 1) % args.checkpoint_frequency == 0:
                save_checkpoint(chk_path, epoch + 1, lr, optimizer, model_pos, min_loss)

            # save best checkpoint
            if e1 < min_loss:
                min_loss = e1
                save_checkpoint(chk_path_best, epoch + 1, lr, optimizer, model_pos, min_loss)
