...
# python deployment/deployment_mesh.py --config configs/mesh/train_h36m.yaml --hyper_cfg hyper_config/method2/manual_joint_label<1 or 2>.yaml --hyper_relation_bias uniform --version 2 --evaluate checkpoint/mesh/h36m/best_epoch_manual<1 or 2>.bin --deployment deployment/output/mesh/h36m/mesh.onnx
# python deployment/deployment_mesh.py --config configs/mesh/train_pw3d.yaml --hyper_cfg hyper_config/method2/manual_joint_label<1 or 2>.yaml --hyper_relation_bias uniform --version 2 --evaluate checkpoint/mesh/pw3d/best_epoch_manual<1 or 2>.bin --deployment deployment/output/mesh/pw3d/mesh.onnx
...
from lib.model.model_mesh_refine import MeshRegressorForDeployment
...
def deploy_with_config(args, opts):
    ...
    model = MeshRegressorForDeployment(args, backbone=model_backbone, dim_rep=args.dim_rep, hidden_dim=args.hidden_dim,
    ...
    input_rand_2d = torch.rand([1, args.clip_len, args.num_joints, 3], requires_grad=False)
    input_hybrik_theta = torch.rand([1, args.clip_len, 72], requires_grad=False)
    input_hybrik_shape = torch.rand([1, args.clip_len, 10], requires_grad=False)

    if torch.cuda.is_available():
        input_rand_2d = input_rand_2d.cuda()
        input_hybrik_theta = input_hybrik_theta.cuda()
        input_hybrik_shape = input_hybrik_shape.cuda()

    os.makedirs(osp.dirname(opts.deployment), exist_ok=True)
    from collections import OrderedDict
    dynamic_axes = OrderedDict({'pose2d': {0: 'batch_size'},
                                'hybrik_theta': {0: 'batch_size'},
                                'hybrik_shape': {0: 'batch_size'},
                                'rot_mat': {0: 'batch_size'},
                                'shape': {0: 'batch_size'},
                                'vertices': {0: 'batch_size'},
                                'keypoint_3d': {0: 'batch_size'},
                                })

    onnx.export(model.module, (input_rand_2d, input_hybrik_theta, input_hybrik_shape), opts.deployment,
                # export_params=True, verbose=False,
                input_names=['pose2d', 'hybrik_theta', 'hybrik_shape'],
                output_names=['rot_mat', 'shape', 'vertices', 'keypoint_3d'], opset_version=12,
...
def test_onnx_ouput(args, deployment_path: str = 'output.onnx'):
    ...
    out_tuple = ort_session.run(
        ['rot_mat', 'shape','vertices', 'keypoint_3d'],
        {
            'pose2d': np.random.rand(1, args.clip_len, args.num_joints, 3).astype(np.float32),
            'hybrik_theta': np.random.rand(1, args.clip_len, 72).astype(np.float32),
            'hybrik_shape': np.random.rand(1, args.clip_len, 10).astype(np.float32),
        })
